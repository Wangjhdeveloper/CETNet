...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        ce_features = feature.unsqueeze(0)
        for layer in self.layers:
            feature = layer(feature, None, mask)
            ce_features = torch.cat((ce_features, feature * mask[:, 0:1, :].unsqueeze(0)), dim=0)
        out = self.conv_out(feature) * mask[:, 0:1, :]

        return out, feature, ce_features
...
class MyTransformer(nn.Module):
    def __init__(self, num_decoders, num_layers, r1, r2, num_f_maps, input_dim, num_classes, channel_masking_rate):
        ...
        self.conv_out = nn.Conv1d(num_f_maps, num_classes, 1)
        self.decoder = Decoder(num_layers, r1, r2, num_f_maps, num_classes, num_classes, att_type='sliding_att',
                    alpha=1)

    def forward(self, x, mask):
        out, feature, ce_features = self.encoder(x, mask)

        outputs = out.unsqueeze(0)

        i = 0
        for cf in ce_features:
            if i != 0:
                out, feature = self.decoder(F.softmax(out, dim=1) * mask[:, 0:1, :], cf * mask[:, 0:1, :], mask)
            i = + 1
            outputs = torch.cat((outputs, out.unsqueeze(0)), dim=0)
        features = nn.functional.normalize(feature).unsqueeze(0)
    ...
    def test(self, batch_gen_tst, epoch):
        ...
        with torch.no_grad():
            while batch_gen_tst.has_next():
                ...
                batch_input, batch_target, mask = batch_input.to(device), batch_target.to(device), mask.to(device)
                p, features = self.model(batch_input, mask)
